  ...
  before: function(xhr, settings) {
    ...
    if (settings.nette && settings.nette.el.attr('data-toggle-tree')) {
      ...
      if (children_block.hasClass('loaded')) {
        if(settings.nette.el.hasClass("toggle-rotate")) {
            settings.nette.el.closest(".datagrid-tree-item").eq(0).addClass("active");
        } else {
            settings.nette.el.closest(".datagrid-tree-item").eq(0).removeClass("active");
        }
  ...
  success: function(payload) {
    ...
    if (payload._datagrid_tree) {
      ...
      for (name in ref) {
        ...
        $('.datagrid-tree-item[data-id=' + id + ']').addClass("active");
      children_block.addClass('loaded');
      children_block.slideToggle('fast');
      $.nette.load();
    }
